 ...
 * No recibe props. Utiliza pasos y formularios seg+¶n el rol seleccionado.
 */
...
const Register = () => {
  ...
  const acceptedFormatsText = 'Aceptamos fotos en JPG, JPEG, PNG, WEBP y HEIC. Si sub+°s otro tipo de archivo (ej: PDF) no vas a poder continuar.';

  const handleNext = (e) => {
    e.preventDefault();
    if (!role) {
      setError('Por favor selecciona qu+Æ tipo de cuenta quieres crear.');
      return;
    }
    setError('');
  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      case 'email':
        ...
        if (v !== v.toLowerCase()) return 'Escribe el correo en min+¶sculas, sin espacios.';
        if (v.includes(' ')) return 'El correo no puede contener espacios.';
        if (!emailRegex.test(v)) return 'Ese correo no parece v+Ìlido. Ej: nombre@ejemplo.com';
        return '';
      case 'password':
        if (!v) return 'Por favor ingresa una contrase+¶a.';
        if (v.length < 8) return 'La contrase+¶a debe tener al menos 8 caracteres.';
        if (v.length > 1000) return 'La contrase+¶a es demasiado larga.';
        if (/^\s+$/.test(value)) return 'La contrase+¶a no puede ser solo espacios.';
        if (detectEmojiOrInvisible.test(v)) return 'La contrase+¶a contiene caracteres inv+Ìlidos.';
        return '';
      case 'dni':
        if (!v) return 'Por favor ingresa tu DNI.';
        if (!/^\d+$/.test(v)) return 'El DNI solo debe tener n+¶meros.';
      ...
      case 'apellido':
        ...
        if (/\d/.test(v)) return 'No uses n+¶meros aqu+°.';
        if (detectEmojiOrInvisible.test(v)) return 'No uses emojis aqu+°.';
        if (v.length > 200) return 'Texto demasiado largo.';
        return '';
      case 'obraSocial':
        if (!v) return 'Por favor indica tu obra social.';
        if (/<script|<\/?[a-z][\s\S]*>/i.test(value)) return 'Nombre inv+Ìlido.';
        if (v.length > 200) return 'Nombre demasiado largo.';
        return '';
      case 'nroAfiliado':
        if (!v) return 'Por favor ingresa tu n+¶mero de afiliado.';
  if (/[^0-9-]/.test(v)) return 'N+¶mero de afiliado inv+Ìlido.';
        if (v.length > 50) return 'N+¶mero de afiliado demasiado largo.';
        return '';
      case 'vencimiento':
        if (!v) return 'Por favor indica la fecha de vencimiento.';
        // Validar YYYY-MM-DD estrictamente y que sea futura y no demasiado lejana (10 a+¶os)
        {
          const parts = String(v).split('-').map(p => Number(p));
          const [y, m, d] = parts;
          if (parts.length !== 3 || !Number.isInteger(y) || !Number.isInteger(m) || !Number.isInteger(d)) return 'La fecha ingresada no es v+Ìlida.';
          const dt = new Date(y, m - 1, d);
          if (dt.getFullYear() !== y || (dt.getMonth() + 1) !== m || dt.getDate() !== d) return 'La fecha ingresada no es v+Ìlida.';
      ...
      case 'fechaNacimiento':
        if (!v) return 'Por favor indica la fecha de nacimiento.';
        {
          ...
          // edad m+Ìxima razonable: 120 a+¶os
      ...
      case 'tarjeta':
        if (!v) return '';
        if (!/^\d+$/.test(v)) return 'La tarjeta debe tener solo n+¶meros.';
        if (v.length < 13 || v.length > 19) return 'La tarjeta debe tener entre 13 y 19 d+°gitos.';
        return '';
      case 'codigoTarjeta':
        if (!v) return 'Por favor ingresa el c+¶digo de la tarjeta (CVV).';
        if (!/^\d+$/.test(v)) return 'El CVV debe tener solo n+¶meros.';
        if (v.length < 3 || v.length > 4) return 'El CVV debe tener 3 o 4 d+°gitos.';
        return '';
      case 'direccion':
        if (!v) return 'Por favor ingresa la direcci+¶n.';
        if (/<script|<\/?[a-z][\s\S]*>/i.test(value)) return 'Direcci+¶n inv+Ìlida.';
        if (v.length > 500) return 'Direcci+¶n demasiado larga.';
        return '';
      case 'coords':
        // value expected as {lat, lng}
        if (!value) return 'No se encontraron las coordenadas. Usa el bot+¶n para buscarlas o completa la direcci+¶n.';
        const latN = Number(value.lat);
        const lngN = Number(value.lng);
        if (!isFinite(latN) || !isFinite(lngN)) return 'Coordenadas inv+Ìlidas.';
        if (latN === 0 && lngN === 0) return 'Coordenadas inv+Ìlidas.';
        return '';
      default:
        return '';
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (role === 'Usuario') {
        ...
        validationErrors.nombreFarmacia = nombreFarmacia ? '' : 'Nombre de farmacia vac+°o';
      ...
      try {
        ...
        if (methods && methods.length > 0) {
          setFieldError('email', 'Este correo ya est+Ì registrado.');
          setError('Este correo ya est+Ì registrado. Usa otro correo o inicia sesi+¶n.');
          return;
        }
      } catch (e) {
      ...
      if (role === 'Usuario' || role === 'Distribuidor') {
        ...
        if (await isDniRegistered(checkDni)) {
          setFieldError('dni', 'Este DNI ya est+Ì registrado.');
          setError('Este DNI ya est+Ì registrado. Si es tu caso, intenta recuperar la cuenta.');
          return;
        }
      }
  ...
  // Variables para almacenar im+Ìgenes base64 de delivery (declaradas aqu+° para que est+Æn
  // visibles tanto dentro del bloque de creaci+¶n como despu+Æs al notificar a farmacias)
      ...
      if (role === 'Usuario') {
        userData = {
          ...
          cobertura,
            ...
            longitud: lng
          ,
          // guardar el string de direcci+¶n original tal como lo ingres+¶ el usuario
          addressString: direccion
        };
        // Simulamos almacenamiento de tarjeta: guardamos versi+¶n enmascarada en la DB
        if (tarjeta) {
          const maskedCard = tarjeta.replace(/\d(?=\d{4})/g, '*');
          const maskedCode = codigoTarjeta ? '***' : '';
          userData.tarjeta = maskedCard;
          userData.codigoTarjeta = maskedCode; // nunca almacenar CVV real en producci+¶n
          try {
            // Simulaci+¶n adicional: almacenar (base64) en localStorage para pruebas
            ...
            console.warn('No se pudo almacenar la tarjeta en localStorage (simulaci+¶n)');
          }
        }
      } else if (role === 'Farmacia') {
        userData = {
          ...
          horarios
          ,
          // guardar el string de direcci+¶n original tal como lo ingres+¶ la farmacia
          addressString: direccionFarmacia
        };
      } else if (role === 'Distribuidor') {
  // convertir imagenes a base64 y guardarlas en DB para revisi+¶n
        try {
          ...
          console.warn('No se pudo convertir las im+Ìgenes a base64', e);
        }
        userData = {
          ...userData,
          dni: dniDelivery,
          ...
          // flag expl+°cita para permitir login cuando la farmacia acepte
          deliveryVerified: false,
          // estado para que el farmac+Æutico pueda aceptar/rechazar
            ...
            Object.entries(usersData).forEach(async ([farmUid, farmData]) => {
              try {
                if (farmData && farmData.role === 'Farmacia') {
                  // crear notificaci+¶n en el nodo de la farmacia
                  await push(ref(db, `notificaciones/${farmUid}`), {
                    tipo: 'delivery_registro',
                    mensaje: `Se registr+¶ un nuevo delivery: ${email}`,
                ...
                console.warn('No se pudo crear notificaci+¶n para farmacia', farmUid, e);
              }
            });
          }
        } catch (e) {
  ...
  setSuccess('Registro exitoso. Ya pod+Æs iniciar sesi+¶n.');
      setTimeout(() => {
        navigate("/");
      }, 1500);
    } catch (err) {
      console.error(err);
      setError('No se pudo crear la cuenta. Intenta de nuevo m+Ìs tarde.');
    }
  };
        ...
        {step === 1 ? (
          ...
          <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button type="button" onClick={() => { setIsFading(true); setTimeout(()=>{ setStep(1); setIsFading(false); }, 300); }}>‘Â… Volver</button>
